...
qu_string1 = """MATCH path = (gene_1:Gene) - [participates_in_9_4_d:participates_in] -> (bioProc_4:BioProc) WHERE gene_1.iri IN $startGeneIris RETURN path
...
"rel_1_9", "rel_9_9_2", "rel_10_10", "rel", "protein_10b", "protein_10", " RETURN path"]



# for w in rep:
#     qu_string = qu_string1.replace(w, "")
# print(qu_string)


qu_string = """MATCH path = (:Gene) - [:participates_in] -> (:BioProc)
MATCH path = (:Gene) - [:part_of] -> (:CoExpCluster) - [:enriched_for] -> (:BioProc)
MATCH path = (:Gene) - [:differentially_expressed] -> (:DGES) - [:enriched_for] -> (:BioProc)
MATCH path = (:Gene) - [:homoeolog] - (:Gene) - [:participates_in] -> (:BioProc)
MATCH path = (:Gene) - [:regulates|genetic|physical] - (:Gene) - [:participates_in] -> (:BioProc)
MATCH path = (:Gene) - [:enc] -> (:Protein) - [:h_s_s|ortho|xref*0..1] - (:Protein) - [:participates_in] -> (:BioProc)
MATCH path = (:Gene) - [:enc] -> (:Protein) - [:h_s_s|ortho|xref*0..1] - (:Protein) - [:has_domain] -> (:ProtDomain) - [:participates_in] -> (:BioProc)
MATCH path = (:Gene) - [:enc] -> (:Protein) - [:ortho] - (:Protein) <- [:enc] - (:Gene) - [:participates_in] -> (:BioProc)
MATCH path = (:Gene) - [:enc] -> (:Protein) - [:ortho] - (:Protein) <- [:enc] - (:Gene) - [:genetic|physical] - (:Gene) - [:participates_in] -> (:BioProc)"""


### create a cypher query to get each pathway counts
qu_string = qu_string.replace("path = (:Gene)", "path = (m:Gene)")
queries_list = qu_string.split("\n")

print("\nCopy the string:\n")
for q in queries_list:
    q = "UNION ALL\n" + q + "\nWHERE m.TAXID = $neodash_gene_taxid" + "\nWITH count(path) AS count" + f"\nRETURN '{q.replace('MATCH path = (m', '(')}' AS pathway, count\n"
    #q = "UNION ALL\n" + q + "\nWHERE m.TAXID = '4565'" + "\nWITH count(path) AS count" + f"\nRETURN '{q.replace('MATCH path = (m', '(')}' AS pathway, count\n"
    print(q)
